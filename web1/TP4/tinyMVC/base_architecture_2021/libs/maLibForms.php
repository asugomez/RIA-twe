...
function mkLigneEntete($tabAsso,$listeChamps=false)
{
	...
	if (!$listeChamps)	// listeChamps est faux  : on utilise le not : '!'
	{
		...
		echo "\t<tr>\n";
		//les indices des cases sont des chaines de caracteres (dans un tableau associatif)
 ...
 * < href='?CIBLE'> Le web en EBM </a>
 * < href='??'> Les qualifs de foot pour la france </a>
 */
function mkLiens($tabData,$champLabel, $champCible, $urlBase=false, $nomCible="")
{
	...
	/*foreach($tabData as $nextData){
		foreach($nextData as $cle => $val){
			if($cle == $champCible){
				if(!$urlBase){
					echo "<a href=$urlBase>$cle</a>";
				}else{
					echo "";
				}
			}
